<template>
	<SiteHeader />
	<Transition appear enter-from-class="opacity-0 -translate-x-64" enter-active-class="delay-200" enter-to-class="opacity-100 translate-x-0">
		<nav
...
const handleMounted = () => {
	...
	const articles = sortBy(
		router.getRoutes().filter(x => x.path !== '/' && x.name !== 'all'),
		'meta.order'
	);
